...
impl PageRequestHandle {
    pub fn new() -> (Self, PageRequest) {
        ...
        let request = PageRequest::new(status_rx, result_rx, cancel);
...
impl Drop for FileRequest {
    fn drop(&mut self) {
        self.cancel.cancel();
    }
}

pub(crate) struct FileRequestHandle {
    pub status_tx: watch::Sender<PageStatus>,
    pub result_tx: Option<oneshot::Sender<Result<Vec<u8>, String>>>,
    pub cancel: CancellationToken,
}

impl FileRequestHandle {
    pub fn new() -> (Self, FileRequest) {
        let (status_tx, status_rx) = watch::channel(PageStatus::RequestingPath);
        let (result_tx, result_rx) = oneshot::channel();
        let cancel = CancellationToken::new();
        ...
        let request = FileRequest::new(status_rx, result_rx, cancel);
