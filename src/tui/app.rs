...
impl TuiApp {
    ...
    fn handle_browser_key(&mut self, code: KeyCode) {
        match code {
            ...
            KeyCode::Enter => {
                if let Some(url) = self.browser.activate() {
                    self.navigate_to_link(&url);
                }
            }
            KeyCode::Backspace => {
                if let Some(url) = self.browser.go_back() {
                    self.navigate_to_link(&url);
                }
            }
            _ => {}
    ...
    fn handle_mouse(&mut self, kind: MouseEventKind, x: u16, y: u16) {
        match kind {
            MouseEventKind::Down(MouseButton::Left) => {
                ...
                match self.mode {
                    AppMode::Browser => {
                        if let Some(url) = self.browser.click(x, y) {
                            self.navigate_to_link(&url);
                        }
                    }
                    AppMode::Normal => {
    ...
    fn navigate_to_link(&mut self, url: &str) {
